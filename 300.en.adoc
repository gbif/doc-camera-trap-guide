...
.Class diagram for camera trap data, constituting of different classes (boxes) and relationships (arrows). Mandatory classes are dark blue, optional classes light blue. The arrow ends represent the cardinality of the relationship, with `○|` zero or one, `○<` zero to many, `|<` one to many, and `||` exactly one. A deployment for example has zero to many media, while a media belongs to exactly one deployment.
...
The media files stored on memory cards should be stored and backed up, preferably by using online services. Due to the large volumes of generated data, storage can require a high investment in terms of space and costs. We recommend to use a <<section-data-management-systems,data management system>> to store your data as some of them offer free storage (e.g. Agouti, Wildlife Insights). It is also useful if your data storage system allows serving images over https, so that they can be linked to in published datasets (see <<table-sequence>>).
...
[#table-deployment]
.Recorded information for the deployment that generated the image in <<figure-example-image>>. https://tdwg.github.io/camtrap-dp/example/00a2c20d/[Source].
[%header,cols=2*]
|===
|Property
|Value
...
[#table-observation]
.Recorded information for one of the observations that is based on the image in <<figure-example-image>>. It is classified at event level (sequence) in the camera trap management system Agouti. https://tdwg.github.io/camtrap-dp/example/00a2c20d/#79204343[Source].
[%header,cols=2*]
|===
|Property
|Value
