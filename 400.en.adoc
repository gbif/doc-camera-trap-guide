...
Data publication is the process of making biodiversity data open and <<FAIR>> cite:[wilkinson-2016]. Adopting the FAIR principles guarantees that your camera trap data can be found, obtained, integrated and reliably reused to benefit various applications in the future (see <<section-fair-data>>). We recommend the use the https://www.gbif.org/ipt[GBIF Integrated Publishing Toolkit (IPT)] cite:[robertson-2014] to publish and register your dataset with the https://www.gbif.org[Global Biodiversity Information Facility (GBIF)]. 
...
Imagine you need to aggregate all observations of muskrats recorded in Belgium in 2023. These data might be scattered across different data sources, hard to access and available in different languages. Making these data sources FAIR means organizing them in a way that everyone can find them, use them and understand the information inside. The best way to start with FAIR data is by making your data findable and accessible. Stable identifiers help people and computers to find and recognize a specific dataset. Add keywords to make people quickly understand what the dataset is about and why they need it.  Register and index the (meta)data in a searchable resource. One easy way to make your data findable and accessible is by depositing your data in a data repository. Repositories are specifically designed for this purpose. to find, aggregate, provide access and store research data. 

The hard part is making sure that the data can be easily used and understood across various systems. We need to determine whether the data are fit for our purpose. The data and metadata should reflect how a camera trap study was planned and deployed, how data was collected and managed. If all these elements are recorded in a way that a human can understand, we make it possible for them to determine whether the data are fit for purpose, or can be mapped into a suitable form. If all these elements are recorded in a way that a machine can interpret (reliable use of identifiers, vocabularies, ontologies, etc.), these data could be useful in unsupervised/semi-supervised modeling and decision-support tools.
