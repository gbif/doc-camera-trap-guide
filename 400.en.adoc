...
With their hierarchical events (<<deployment,deployments>>, <<sequence,sequences>>) and resulting <<observations>>, it seems logical to express camera trap data as a https://www.gbif.org/sampling-event-data[[Sampling-event dataset]] with an Event core (see <<table-event-core>> and Occurrence extension (see <<table-occurrence-extension>>). But doing so impedes us from expressing the <<media>> information as an extension, since the star-schema design of Darwin Core Archives does not allow relating an extension to an extension. The only available option would be to express all image information in term:dwc[dwc:associatedMedia], which would reduce it to a (list of) URL(s).
