...
The actions taken to make the published data less specific or complete than in its original form. We recommend to succinctly describe here what <<section-sensitive-information,sensitive information>> of the record was generalized and how. Note that this information can be provided at record level and does not need to apply to the whole dataset. If important information was omitted altogether, use term:dwc[dwc:informationWithheld].
...
The name or identifier of the person or <<AI>> algorithm that assigned the <<section-scientificname,scientificName>> to the observation. We recommend providing a single name: that of the person/algorithm that most recently <<classification,classified>> the observation. This information is typically recorded by the <<data-management-system,data management system>>. Note that this term contains <<section-personal-data,personal data>>. In Camtrap DP, this term corresponds with {observations-classifiedBy}.
...
The date or datetime on which the <<section-scientificname,scientificName>> was assigned to the observation. We recommend providing a single timestamp: that of the <<classification>> made by the person/algorithm indicated in <<section-identifiedby,identifiedBy>>. This information is typically recorded by the <<data-management-system,data management system>>. Write timestamps in the ISO 8601 format (`YYYY-MM-DDTHH:MM:SS`) and include the timezone (`+02:00`) or convert and indicate as UTC (`Z`). In Camtrap DP, this term corresponds with {observations-classificationTimestamp}.
