...
Not all information in a published Camtrap DP is currently harvested by GBIF. The current GBIF data model requires it to be processed to Darwin Core before ingestion. This process is provided by the https://inbo.github.io/camtraptor/reference/write_dwc.html[write_dwc] function in the R software package camtraptor cite:[camtraptor]. This function implements the <<section-darwin-core-archive,Darwin Core Archive>> recommendations suggested in this document. GBIF will be able to process more information from a published Camtrap DP once it has implemented a new data model cite:[gbif-2022].
