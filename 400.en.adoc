...
Note that the <<ac-extension>> can contain duplicates, an important difference with {media} where each file is only listed once. Repeated term:dwc[dwc:occurrenceID] are the result of a single <<section-media-or-event-based-classification,event based>> observation being related to multiple media files (e.g. observation `05230014` in <<table-ac-extension>>). Repeated https://ac.tdwg.org/termlist/#dcterms_identifier[dcterms:identifier] are the result of a media file being the source for multiple observations (e.g. multiple species observed in the same image, such as in media `e68deaed` in <<table-ac-extension>>). The extension should however contain unique `observationID`+`identifier` combinations.
