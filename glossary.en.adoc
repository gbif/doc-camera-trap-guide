...
[[sensor]]sensor:: Device that detects changes in the environment, such as movement, heat, light, sound, or other stimuli.
...
[[trigger]]trigger:: Condition that prompts a <<camera>> to activate and capture <<media>>. Modern camera traps typically use an integrated passive infrared (PIR) sensor that is designed to detect <<activity>> based on a combination of heat and motion.
