...
[[trigger]]trigger:: Device that detects changes in the environment, such as movement, heat, light, sound, or other stimuli. They trigger the capture of <<image,images>>, <<video,videos>>, or audio recordings when specific conditions occur within the camera's field of view.
